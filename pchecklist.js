...
var passwordCheckOld = function () 
{
  ...
  if (stringPassword.length < 8) //Condition checks to see that the password is atleast 8 characters long
  {
  stringOutput = "This password is too short \n Please ensure your password is atleast 8 characters long";  
  } 
    else // If the if-statement condition is false enters the for-loop to check password against array list
    {
        for (var a = 0; a < arrayBadPasswords.length; a++) //Condition to check through the entire list of the array 
        {
            if (stringPassword === arrayBadPasswords[a]) //Condition to check to see if the password matches the array entry. If it does match error output.
            {
                stringOutput = "This password matches one of the 500 Bad passwords \n you want to avoid using. \n Please try again.";
                break;
            }
                else
                {
                    stringOutput = "Great Password!"; //Condition was false. stringPassword passes checks.
                }
        }
        
    }
    $("output").value = stringOutput;// Outputs 1 of the 3 stringOutputs depending on what conditions were true and false
};
